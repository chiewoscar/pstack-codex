...
When the host permits parallel agents, start independent, read-only explorers with the current host's subagent spawn tool, up to the available slot limit. Omit `model` unless a valid override is explicitly configured. Otherwise explore the slices serially.
...
Explore and explain in the current task. A narrow question does not need a subagent.
...
Once all explorers return, synthesize their findings into one coherent explanation in the current task.
...
After the explanation, use up to three independent read-only critics when the host permits and the scope warrants them. Give each a different architectural lens. Use the current host's subagent spawn tool and batch at the host's slot limit. Omit model overrides by default. If delegation is unavailable, apply the lenses serially and say so.
