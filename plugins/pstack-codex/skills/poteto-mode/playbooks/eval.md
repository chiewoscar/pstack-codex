...
4. **Spawn N parallel candidates** with distinct approaches per the **arena** skill's Phase B; use different models only when available and authorized. Each works in its own sanitized dir; same prompt to each.
5. **Spawn one blinded judge** with a separate judging lens per the **arena** skill's Phase C; a different model family is optional and host-dependent. Judge sees outputs by sanitized label and the rubric, never a model name.
6. **Verify the chain from transcripts, not self-report.** Use the current task's visible tool record or an exact task export when available. Do not search raw transcript directories across projects. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
