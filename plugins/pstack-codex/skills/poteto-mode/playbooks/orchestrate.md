...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code: conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap; queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Choose one of the authorized worker modes in `../../../references/codex-runtime.md`; same-task children use Codex collaboration tools, while configured remote workers use Codex Cloud task IDs. State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out, to conserve context. The bookkeeping CLI never spawns, waits, or wakes agents.
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer: each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers only when the host permits nested delegation. Rolls up aggregates at wave boundaries; never forwards raw child reports. Cap in-flight children at the host's stated concurrency limit.
- **Worker / verifier.** Same-task Codex subagents share the local filesystem. Give each writer its own verified worktree or file scope. A read-only prompt does not enforce a sandbox; use an independent read-only CLI session or an appropriately scoped separate environment when enforcement is required. Remote workers require an authorized Codex Cloud environment and a pushed branch. Prefer fewer, broader workers. Use separate review lenses; model diversity is optional and host-dependent.
...
- `inbox/` holds completion pointers. `gates.md` parks human gates (question, options, default on no answer) so a completion flood cannot wipe request_user_input_async state.
...
- Exactly one stacker per stack may run `gt`, serialized within its stack; record the holder in the standing orders. Run a large restack in an authorized, configured Cloud environment when available. If no suitable environment exists, plan a bounded local window or report the infrastructure blocker instead of assuming a remote agent exists.
...
Scale verification to the unit. When VERIFY is a single cheap command, the worker runs it and reports the output, and the coordinator spot-checks receipts; a dedicated verifier agent (with a distinct review lens; model diversity is optional and host-dependent) is for units whose verification is expensive, judgment-laden, or high-blast-radius. A verifier agent whose entire product would be rerunning one command is ceremony, not verification.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the available collaboration status tools. Transcript mtime is not liveness.
