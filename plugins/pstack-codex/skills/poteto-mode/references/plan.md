...
State your read of scope and constraints in one paragraph. Use `request_user_input_async` only for genuinely ambiguous intent (the **never-block-on-the-human** principle skill); give concrete options with each open question.
...
- If delegation is permitted, use the current host's subagent spawn tool and point it to `references/poteto-agent.md`. No agent type parameter exists.
- Omit `model` by default; use only host-supported overrides explicitly requested by the user or host.
...
If a phase creates or edits a skill, the phase instructs the implementer to use the **skill-creator** skill (Codex's `$skill-creator` skill).
...
- `$pstack-codex:deslop` over each diff before commit. the **unslop** skill over any prose surface.
- the **show-me-your-work** skill to keep a decision trail when the plan is large enough to need an auditable record.
- one bounded PR state and checks verification after opening; use the **Babysit** playbook (`playbooks/babysit.md`) only when the user asks to watch or get the PR green.
