...
Search the evidence categories relevant to the question and available in this host. Begin with source control, then expand when the answer remains uncertain or the stakes warrant it. Record which categories were not searched.

### Discovery

Before investigating, inspect the available Codex tools, app connectors, and MCP resources. Do not assume that a named integration is installed or connected.
...
If independent categories merit parallel work and delegation is allowed, launch at most the host's available slots and drain them before the next batch.

For bounded independent categories, use the current host's subagent spawn tool only when permitted by the host. Say in each prompt that the work is read-only. Subagents share the host's tools; verify actual connector access rather than assuming a special agent type. Otherwise investigate serially.
...
Synthesize in the current task and spot-check the citations. An authorized independent synthesizer is optional when the evidence set is large.
