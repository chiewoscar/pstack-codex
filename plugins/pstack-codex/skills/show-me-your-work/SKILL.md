...
description: "Use only when explicitly invoked or routed by another pstack skill. Keep a reviewable decision trail for long-running or unattended work: a TSV log with one row per decision (what, why, evidence, result). Local by default; commit it when a reviewer needs the trail to trust the result. Use for /show-me-your-work, autonomous or multi-phase runs, or work a human reviews after stepping away."
---
...
At the end of the run, before handing back, check the log told the truth. Compare the log with the visible current task and tool results. If a task export is available, use only this task's export. Do not search raw transcript directories across projects. Walk the log against what actually happened:
...
Before handing back, request a bounded read-only reviewer when delegation is authorized and available. Give it the audit trail and the visible task digest or an available export. Use a different model family only when this host offers one and a user or host setting permits the override; otherwise use a distinct review lens and name the narrower independence. If delegation is unavailable, audit locally and say so. The review flags what the user should pay attention to, not a redo of the work.
...
Every reply for a run that produced a trail ends with an "Attention" section. Lead with the reviewer's identity and model when known (`reviewed by <reviewer>, <model>`), or `self-audited` when no reviewer was available, then list flags pointing to specific rows or moments. `No flags` is a valid value. The self-audit asks if the log told the truth; this asks what the user should still scrutinize even when it did.
