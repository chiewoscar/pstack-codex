---
name: figure-it-out
description: "Use only when explicitly invoked or routed by another pstack skill. Design an auditable playbook when no narrower one fits: a large migration, an ambitious multi-part change, or work a human reviews after stepping away. Scales rigor to the task, runs a hypothesis loop, and logs decisions via show-me-your-work. Use for /figure-it-out, 'figure it out', a large migration, or when no narrower playbook applies."
---
...
- For one-way-door design decisions, run the **architect** skill (it runs **arena**) with distinct design briefs, separate worktrees or output paths for writers, and a read-only judge. Use different model families only when the host offers them and the user or host explicitly authorizes overrides. Skip it for mechanical work whose shape is already concrete. A second arena over a settled design is over-engineering (the **laziness-protocol** principle skill).
- Decide what fans out. Parallelize only across genuine seams, and give each writer its own verified worktree or output path (the **separate-before-serializing-shared-state** principle skill). A branch name alone does not isolate files. Don't over-fan.
