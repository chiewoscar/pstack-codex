---
name: recall
description: "Use only when explicitly invoked or routed by another pstack skill. Reconstruct your recent working context from your own chat history, live state, and the shared record (user reports, prior fixes, incidents), then hand back a tight current-state brief. Use for 'recall my work on X', 'catch me up', 'what have I been working on', 'where did I leave off', before starting or resuming work."
---
...
Keep it tight and on-topic. Read only what the in-scope threads need, then stop. For large histories, bounded read-only subagents may summarize separate slices when the host permits. Otherwise inspect the relevant tasks serially.
...
Use the current conversation and available Codex task tools (`list_threads`, `read_thread`) for prior task history. Scope searches to the active project or the user's named topic. Do not guess transcript file paths.
...
3. Use `list_threads` to identify in-scope tasks by project, date, and topic, then `read_thread` for the relevant turns. For large histories, authorized read-only subagents may inspect separate task IDs. Each returns: topic, user goal, decisions, open work, corrections, and artifacts, citing the task ID. For one or two tasks, inspect them directly. Do not mine unrelated projects.
...
5. Verify against live state. A transcript or a stale ticket is history, not current truth, so take the PRs, branches, and tickets that the mining and the sweep surfaced and check them with `git` and `gh`. When the answer hinges on exact tool actions, request outputs for the relevant task if the tool supports them. Otherwise state the evidence gap.
